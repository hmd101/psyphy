...
if fitted is None:
    raise RuntimeError(
        "Posterior missing 'params' attribute; expected by this example."
    )
...
Sigma_fit_plot = Sigma_fit + (float(noise.sigma) ** 2) * np.eye(
    2
)  # fitted covariance with noise
Sigma_init = np.diag(np.exp(np.array(init_params["log_diag"], dtype=float)))
Sigma_init_plot = Sigma_init + (float(noise.sigma) ** 2) * np.eye(
    2
)  # initial covariance with noise
Sigma_true_plot = Sigma_true + float(noise.sigma) ** 2 * np.eye(
    2
...
fig.savefig(
    os.path.join(PLOTS_DIR, "offline_fit_mvp_mapopt_thresholds.png"),
    dpi=200,
    bbox_inches="tight",
)
...
if steps_hist and loss_hist:
    ...
    fig2.savefig(
        os.path.join(PLOTS_DIR, "offline_fit_mvp_mapopt_learning_curve.png"),
        dpi=200,
        bbox_inches="tight",
    )
else:
